...
class Slots(object):
    ...
    def get_stats(self, retry_delay=30, max_retries=4, constraint=None, extra_stats=None):
        ...
        while retries < max_retries:
            try:
                ads = self.collector.query(htcondor.AdTypes.Startd, constraint, stats)
            except:
                logger.warning("Trouble getting pool {0} startds, retrying in {1}s.".format(self.pool,retry_delay))
                retries += 1
                ads = None
        ...
        if ads is None:
            logger.error("Trouble getting pool {0} startds, giving up.".format(self.pool))
            return
        ...
        for r in ads:
            ...
            for s in ["DaemonStartTime","GLIDEIN_ToDie"]:
                if s in rdict:
                    rdict[s] = datetime.datetime.fromtimestamp(rdict[s]).isoformat()
            if "Memory" in rdict:
            ...
            if "GLIDEIN_ToDie" in r and "MyCurrentTime" in r:
                ...
                rdict["time_left_8hours"]=math.floor(time_left/3600/8)*8
            ...
            if rdict.get('GLIDEIN_Site') == "FNAL":
                rdict["GLIDEIN_Site"] = rdict.get("GLIDEIN_ResourceName","FNAL")
            yield rdict
